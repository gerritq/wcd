...
TEST_DIR = os.path.join(BASE_DIR, "scripts/rebuttal/llm_test")
SHOTS_PATH = os.path.join(BASE_DIR, "scripts/rebuttal", "all_shots.json")
...
def get_balanced_shots(lang_shots: List[Dict], shots_n: int) -> List[Dict]:
    if shots_n == 0:
        return []
    if shots_n < 0 or shots_n % 2 != 0:
        raise ValueError("Shots must be a non-negative even integer.")

    per_label = shots_n // 2
    label_1 = [s for s in lang_shots if int(s["label"]) == 1][:per_label]
    label_0 = [s for s in lang_shots if int(s["label"]) == 0][:per_label]

    if len(label_1) < per_label or len(label_0) < per_label:
        raise ValueError(
            f"Not enough shots for balanced selection: requested {per_label} per label, "
            f"got label_1={len(label_1)} and label_0={len(label_0)}."
        )

    shots = []
    for i in range(per_label):
        shots.extend([label_1[i], label_0[i]])
    return shots
    ...
    if args.shots > 0:
        def format_few_shot(system_message: str, 
                ...
                shots = [
                            ...
                            f"Claim: {s['claim']}\n"
                            f"Subsequent sentence: {s['subsequent_sentence']}\n"
        ...
        with open(SHOTS_PATH, "r", encoding="utf-8") as f:
            lang_shots = json.load(f)[args.lang]
        selected_shots = get_balanced_shots(lang_shots=lang_shots, shots_n=args.shots)
        rows = [format_few_shot(system_message, ex, selected_shots) for ex in data]
    else:
...
def main() -> None:
    ...
    assert args.shots >= 0 and args.shots % 2 == 0, "Shots must be a non-negative even integer."
