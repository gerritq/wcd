...
def get_devset(args, tokenizer):
    ds = load_from_disk(os.path.join(DATA_DIR, args.lang))
    dev = ds['dev']
    dev = dev.map(preprocess_function_generation, 
                      fn_kwargs={"tokenizer": tokenizer})

    text = list(dev['text'])
    labels = list(dev['label'])
...
def get_config(args, tokenizer):
    ...
    training_args = SFTConfig(
        ...
        gradient_accumulation_steps=args.grad_accum, #2 before          # number of steps before performing a backward/update pass
...
def inference(data, model_dir, tokenizer, bnb_config, batch_size=8):
    """
    ...
    # batch inference
    predictions = []
    for i in tqdm(range(0, len(data), batch_size), desc="Running batch inference ..."):
...
def clean_folder(folder_path):
    for item in os.listdir(folder_path):
        ...
        if os.path.isfile(path) or os.path.islink(path):
            ...
            shutil.rmtree(path)
    print(f"All items cleared in {folder_path}")
...
def get_run_data(model_dir):
    ...
    if os.path.getsize(meta_path) == 0:
        return 1, 0.0  # model_1, best accuracy = 0.0
    model_numbers = []
    accuracies = []
    ...
    with open(meta_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            model_numbers.append(entry["model_number"])
            accuracies.append(entry["dev_metrics"][0]["accuracy"])
        
    next_model_num = max(model_numbers) + 1 if model_numbers else 1
    best_dev_acc = max(accuracies) if accuracies else 0.0

    return next_model_num, best_dev_acc
...
def main():
    ...
    parser.add_argument("--plw", type=int, required=True)
    ...
    print(f"\t LR {args.learning_rate} - BATCH {args.batch_size} - EPOCHS {args.epochs} - LORA_R {args.lora_r}")
    ...
    collect_and_save_losses(trainer.state.log_history, TEMP_DIR)

    # EVAL
    tokenizer = get_tokenizer(args, inference=True)
    test, test_labels = get_testset(args, tokenizer)
    dev, dev_labels = get_devset(args, tokenizer)
    
    test_predictions = inference(test, TEMP_DIR, tokenizer, bnb_config)
    test_metrics = evaluation(test_predictions, test_labels)

    dev_predictions = inference(dev, TEMP_DIR, tokenizer, bnb_config)
    dev_metrics = evaluation(dev_predictions, dev_labels)

    # Save to meta
    model_number, best_dev_acc = get_run_data(MODEL_DIR)
    meta = {
    ...
    # update best model
    if dev_metrics[0]['accuracy'] > best_dev_acc:
