...
def create_zero_shot_df(rows: list[dict]) -> pd.DataFrame:
    ...
    # save
    zero_results_check.to_excel("checks/zero_shot_results_check.xlsx")
...
#     table = "\n\n"
#     colspec = "ll" + "Cc" * len(MODEL_ORDER)

#     header = " & & & \\multicolumn{" + str((len(df.columns) - 3)*2) + "}{c}{\\textbf{SFT Variant}} \\\\ \n"
#     header += "\\cmidrule(lr){4-" + str(len(df.columns)*2) + "} \n"
...
#         # add models with diff and std
#         for model in MODEL_ORDER:
#             mean = row[f"{model}_mean"]
#             std = row[f"{model}_std"]

#             if "mbert" in model.lower():
#                 line += f"& {mean:.2f} \\scriptsize{{(±{std:.2f})}} "
#             else:
#                 diff = mean - row["mBert_mean"]
#                 line += (
#                     f"& {mean:.2f} \\scriptsize{{(±{std:.2f})}} "
#                     f"& (\\posneg{{{diff:.2f}}}) "
#                 )
...
def main():
    ...
    # few-shot lr table
    if ITEM == "LR" or ITEM == "ALL":
        df_few_shot, df_learning_rate, pivoted = create_few_shot_df(rows)
        if ITEM == "LR":
            print("="*60)
            print("METRIC: ", METRIC)
            print("="*60)
        ...
        if ITEM == "ALL":
            ...
            print("METRIC: ", METRIC)
            print("="*60)
            # all_results_latex_table(pivoted)  
            generate_main_figure(df_few_shot)
