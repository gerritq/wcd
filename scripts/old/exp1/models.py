...
from prompts import VANILLA_PROMPTS
...
MODEL_DIR = os.path.join(BASE_DIR, "data/exp1")
...
class LM(ABC):
    ...
    def __init__(self, 
        ...
        self.data_dir = os.path.join(DATA_DIR, "main")
        ...
        # Data
        # Select vanilla or explnation template
        self.prompt_template = VANILLA_PROMPTS[self.lang]

        # Select claim only or with context
        self.prompt_template['user'] = (self.prompt_template['user_context'] if self.context 
                                        else self.prompt_template['user_claim'])
    ...
    def _resample_train_data(self, total_size: int) -> Dataset:
        ...
        for x in combined:
            if x['label'] == 1:
                ...
                neg+=1
        assert pos == neg, "Dataset unbalanced after resampling."
