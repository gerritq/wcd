...
def preprocess_function_training(example: Dict, 
    ...
    user = prompt_template[lang]['user'].format(**example)
    assistant = prompt_template[lang]['assistant'].format(**example)
...
def evaluation(ds: Dataset, 
        ...
        for i in tqdm(range(0, len(ds), batch_size), desc="Running batch inference ..."):
            ...
            with torch.no_grad():
                out = model.generate(
                    ...
                    max_new_tokens=32 if binary else 256, # higher for explanations
                    temperature=.1,
                    )
    ...
    # run here
    predictions, labels = inference(ds, model_path, tokenizer_eval, bnb_config=bnb_config)
